...
void handle_client(int client_fd) {
    ...
    // Recieve  welcome message to client
    ...
    printf("C: %s\n", buffer); // Printing client command

    //Replyong to hello command
    memset(buffer,0,BUFFER_SIZE);
    ...
    //Extract username of sender from the client message ans replying
    ...
    printf("C: %s\n", buffer); // Printing client command
    //extracting reciever username from client command
    char username_recv[BUFFER_SIZE];
    if (sscanf(buffer, "USER <%[^@]", username_recv) != 1) {
    ...
    //printing the content sent by client
    ...
    while (1) {
        ...
        if(i!=1){
        printf("C: %s", buffer); // Printing client command
        }
        i++;
        if (strcmp(buffer, ".\n") == 0) {
            break;
        }
        // Writing received message to mymailbox file
        fprintf(mymailbox_file, "%s", buffer);
    }
    fprintf(mymailbox_file, "%s", "\n\n");
    fclose(mymailbox_file);
    // Sending message accepted response
    memset(buffer,0,BUFFER_SIZE);
    ...
    //sending closing connection message
    memset(buffer,0,BUFFER_SIZE);
...
int main(int argc,char* argv[]){
    ...
    //smtp port number from command line argument
    ...
    while (1) {
        ...
        if (pid < 0) {
            ...
            // Child process
            close(server_fd); // Closing  the listening socket in the child process
            handle_client(client_fd); 
            exit(EXIT_SUCCESS);
        } else {
            // Parent process
            close(client_fd); // Closing the client socket in the parent process
        }
    }
